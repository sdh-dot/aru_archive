...
class ClassifyPreviewDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        hdr.setStyleSheet(
            "font-weight: bold; font-size: 12px; color: #E69AAA;"
        )
        layout.addWidget(hdr)

        src_path_lbl = QLabel(self._preview["source_path"])
        src_path_lbl.setWordWrap(True)
        src_path_lbl.setTextInteractionFlags(
            Qt.TextInteractionFlag.TextSelectableByMouse
        )
        src_path_lbl.setStyleSheet(
            "font-size: 10px; color: #D8AEBB; padding-bottom: 4px;"
        )
        layout.addWidget(src_path_lbl)

        # 복사 예정 목록
        dest_hdr = QLabel("복사 예정:")
        dest_hdr.setStyleSheet(
            "font-weight: bold; font-size: 11px; color: #E69AAA;"
        )
        ...
        if dests:
            for i, d in enumerate(dests, 1):
                ...
                lines.append(f"{i:2}. [{rule}] {d['dest_path']}{extra}  {will}")
        else:
            lines.append("(복사 대상 없음 — 분류 가능한 태그/작가 정보가 없거나 classified_dir 미설정)")
        ...
        summary = QLabel(
            f"예상 복사본: {copies}개    예상 용량: {size_str}"
        )
        summary.setStyleSheet(
            "font-size: 11px; color: #8FD694; padding-top: 4px;"
        )
